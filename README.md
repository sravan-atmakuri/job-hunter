...
# 2. Run setup (installs everything, and generates the run.sh helper used below)
bash setup.sh
...
> `run.sh` does not exist in a fresh clone — **it is created by `setup.sh`** (step 2). It activates the virtual environment and runs `main.py` (or any script you pass it).
...
:: 2. Run setup (installs everything, and generates the run.bat helper used below)
setup.bat
...
> Likewise, `run.bat` is generated by `setup.bat` in step 2.
...
Prefer a browser over the terminal? A Flask app wraps the core workflow — scrape jobs, upload a resume, analyze it against a posting, generate a tailored version, and kick off auto-apply, all from a local web page:
...
> `launch_apply.sh` assumes macOS and Chrome profile "Profile 1" — edit the `CHROME` and `PROFILE` variables at the top of the script if your setup differs.
